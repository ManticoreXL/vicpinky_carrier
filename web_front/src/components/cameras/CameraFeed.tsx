...
export default function CameraFeed({ botId, label, socket }: Props) {
 ...
 {/* 비스트리밍 오버레이 — 신호 없을 때 깔끔한 플레이스홀더(카메라 미연결도 자연스럽게) */}
 {status !== "streaming" && (
 <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-black/25 backdrop-blur-sm">
 <svg className="w-7 h-7 text-white/[0.28]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
 d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
 </svg>
 <p className={`text-xs font-semibold tracking-wide ${
 status === "requesting" || status === "connecting" ? "text-white/[0.6] animate-pulse" : "text-white/[0.45]"
 }`}>
 {status === "requesting" ? "연결 중…" :
 status === "connecting" ? "협상 중…" :
 "신호 없음"}
 </p>
 {(status === "idle" || status === "error") && (
 <button
 onClick={status === "idle" ? connect : cleanup}
 disabled={!socket}
 className="px-3 py-1 text-xs font-semibold tracking-wide rounded-md
 border border-white/[0.12] text-white/[0.6]
 hover:border-white/[0.25] hover:text-white/[0.85] transition-all
