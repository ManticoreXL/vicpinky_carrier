...
### FMS Dashboard
현장 상황을 실시간으로 파악하고 다수 로봇의 상태를 모니터링·제어하는 통합 관제 시스템입니다. 관제를 넘어 다중 로봇을 실제로 운용하는 Fleet Management System 역할을 수행합니다. 로봇과는 rosbridge, 프론트엔드와는 Socket.IO로 실시간 통신하며, 대시보드는 제어, 태스크, 테스트, 관리 화면으로 구성되어 로봇 운용의 전 과정을 한 곳에서 다룰 수 있습니다.

<div align="center">
  <table>
    <tr>
      <td align="center">
        <img src="assets/features/web/web_control.png" width="100%"/><br/>
        <sub><b>제어 화면</b></sub>
      </td>
    </tr>
  </table>
</div>
개별 로봇을 직접 조작하고 상태를 실시간으로 관찰하는 화면입니다. 왼쪽 Fleet Units 목록에서 로봇을 선택하면 해당 로봇의 정보가 표시됩니다. 중앙 상단에는 Odometry, IMU, LiDAR Scan, 배터리(battery_state) 등 실시간 센서 데이터가 나타나고 cmd_vel을 직접 주입할 수 있습니다. 그 아래 제어 패널에서는 WASD 키보드 주행, 자기진단, 리플레이(Replay), 라인트레이싱(LineTrace), 음성 발행(/speak_cmd) 등 각 로봇에 대한 제어 명령을 내릴 수 있습니다. 오른쪽에는 WebRTC 기반 실시간 카메라 영상이 스트리밍되어 로봇 시점을 직접 확인할 수 있습니다.

<div align="center">
  <table>
    <tr>
      <td align="center">
        <img src="assets/features/web/web_task.png" width="100%"/><br/>
        <sub><b>태스크 화면</b></sub>
      </td>
    </tr>
  </table>
</div>
로봇에게 수행할 임무를 부여하고 진행 상황을 관리하는 화면입니다. 오른쪽 '새 태스크' 패널에서 단일, 연속, 시나리오 모드를 선택하고 이동, 구호, 충전, 쿼리, 일시정지와 순찰 등 세부 작업을 조합해 임무를 구성한 뒤 원하는 로봇에 급파합니다. 왼쪽에는 전체 로봇의 임무를 모아 보는 글로벌 태스크 큐가, 중앙에는 로봇별 임무 목록과 진행률이 표시됩니다. 생성된 태스크는 적합한 로봇에 자동 배차되어 최단 경로로 주행하며, 경로상 노드 점유를 감지해 충돌을 방지하고 장애가 발생한 노드는 폐쇄해 우회시키는 등 전 과정이 자동으로 처리됩니다. 또한 로컬 언어 모델 AI를 연동해 자연어 명령을 태스크로 변환하고, 현재 로봇들의 상황을 반영한 브리핑을 제공할 수 있습니다.

<div align="center">
  <table>
    <tr>
      <td align="center">
        <img src="assets/features/web/web_test.png" width="100%"/><br/>
        <sub><b>테스트 화면</b></sub>
      </td>
    </tr>
  </table>
</div>
실제 하드웨어 없이 FMS의 동작을 검증하기 위한 가상 로봇 테스트 화면입니다. TEST-BOT과 같은 테스트용 로봇을 배치하고 배터리 잔량을 슬라이더로 조절하거나 상태 신호를 발생시킬 수 있습니다. 전복 신호 발생, 배터리 부족 경고, 충전 완료 보고 등 다양한 상황을 임의로 만들어, 관제 서버가 로봇의 이상 상태를 스스로 감지하고 태스크를 재스케줄링하는 장애 대응 로직을 안전하게 시험할 수 있습니다.

<div align="center">
  <table>
    <tr>
      <td align="center">
        <img src="assets/features/web/web_manage.png" width="100%"/><br/>
        <sub><b>관리 화면</b></sub>
      </td>
    </tr>
  </table>
</div>
시스템을 구성하는 데이터를 등록하고 관리하는 화면입니다. 토폴로지 편집기, 로봇, 맵(FleetMap), 노드, 엣지, 태스크 탭으로 나뉩니다. 로봇 등록부에서는 robot_id, IP, 도메인 ID, 상태, 맵, 노드 정보를 확인하고 로봇을 추가·수정·삭제할 수 있으며, 맵과 노드, 엣지로 이루어진 토폴로지, 태스크 정의 또한 이 화면에서 관리합니다. 수집된 맵은 노드-엣지 그래프로 관리되어 태스크 배차와 경로 계획의 기반이 됩니다.
