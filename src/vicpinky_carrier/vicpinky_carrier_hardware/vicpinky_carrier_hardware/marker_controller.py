...
class CentralParkingController(Node):
    def __init__(self):
        ...
        self.tb3_01_stage = 'PARKING'
        self.tb3_02_stage = None
        self.tb3_03_stage = None
        self.tb3_04_stage = None
        ...
        self.tb3_01_sub = self.create_subscription(RobotState, '/tb3_01/robot_state', self.tb3_01_state_callback, qos)        
        self.tb3_02_sub = self.create_subscription(RobotState, '/tb3_02/robot_state', self.tb3_02_state_callback, qos)        
        self.tb3_03_sub = self.create_subscription(RobotState, '/tb3_03/robot_state', self.tb3_03_state_callback, qos)        
        self.tb3_04_sub = self.create_subscription(RobotState, '/tb3_04/robot_state', self.tb3_04_state_callback, qos)        
    ...
    def _fix_marker(self, cx, cy, th, mid):
        ...
        if self.tgt_x[4] is not None and self.tgt_x[3] is not None and self.tgt_x[2] is not None and self.tgt_x[0] is not None:
            ...
            self.get_logger().info(f"🔒 [All targets locked] : {self.tgt_x[4]}, {self.tgt_x[3]}, {self.tgt_x[2]}, {self.tgt_x[0]}")

    def tb3_01_state_callback(self, msg):
        self.tb3_01_stage = msg.stage

    def tb3_02_state_callback(self, msg):
        self.tb3_02_stage = msg.stage

    def tb3_03_state_callback(self, msg):
        self.tb3_03_stage = msg.stage

    def tb3_04_state_callback(self, msg):
        self.tb3_04_stage = msg.stage
    ...
    def camera_loop(self):
        ...
        if ids is not None:
            ...
            for i, mid in enumerate(id_list):
                ...
                if mid in self.robot_id_list:
                    if (self.tb3_04_stage == 'LOADING' 
                        or self.tb3_04_stage == 'TRACE' 
                        or self.tb3_04_stage == 'PARKING'):
                        if mid == 4:
                            self.current_robot = mid
                    elif (self.tb3_03_stage == 'LOADING' 
                        or self.tb3_03_stage == 'TRACE' 
                        or self.tb3_03_stage == 'PARKING'):
                        if mid == 3:
                            self.current_robot = mid
                    elif (self.tb3_01_stage == 'LOADING' 
                        or self.tb3_01_stage == 'TRACE' 
        ...
        if self.tgt_locked and self.current_goal is not None:
            ...
            wp_x = self.tgt_x[self.goal_index] - self.WAYPOINT_OFFSET * cos_t
            wp_y = self.tgt_y[self.goal_index] - self.WAYPOINT_OFFSET * sin_t
        ...
        self._show_frame(frame)
        print(f"[TRACK] 6. 루프 정상 완료 {self.current_goal}", flush=True)
